...
// Define a class to manage the state of projects
class ProjectsNotifier extends StateNotifier<List<ProjectModel>> {
  ...
  List<ProjectModel> getProjectsByCategory(String category) {
    return state.where((project) => project.category == category).toList();
  }

  // Method to get projects by user ID
  List<ProjectModel> getProjectsByUserId(String userId) {
    return state.where((project) => project.userId == userId).toList();
  }
}

// Create a provider that manages a list of ProjectModel objects
final projectsProvider =
    StateNotifierProvider<ProjectsNotifier, List<ProjectModel>>(
  (ref) => ProjectsNotifier(),
);

// Initial list of projects (your dummy data)
final List<ProjectModel> _initialProjects = [
  ProjectModel(
    id: 'project1',
    userId: 'user123',
    name: 'Tetris Game',
